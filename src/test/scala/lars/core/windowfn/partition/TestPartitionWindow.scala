...
class TestPartitionWindow extends FunSuite {
  ...
  /*Test from reactknow 2014*/
  test("test exp5") {

    def idx(a:Atom): Int = a match {
      case tram(vehicleId, station) => 1
      case _                        => 2
    }

    def n(i: Int): TupleWindowFixedParams = i match {
      case 1 => TupleWindowFixedParams(TupleWindowParameters(2,0))
      case _ => TupleWindowFixedParams(TupleWindowParameters(0,0))
    }
    ...
    val T = Timeline(0,13)
    val s = S(T) + (2->tr1) + (2->b1) + (8->tr2) + (11->b2)

    val w = PartitionWindow.fix(idx,n)

    assert(w(s,11) == (S(T) + (2->tr1) + (8->tr2)))
  }
}
